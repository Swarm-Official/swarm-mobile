...
impl<P, U> Builder<P, U> {
    ...
    fn check_version_compatibility<FE>(&self, version: TxVersion) -> Result<(), Error<FE>> {
...

        {
            ...
            // NOTE: this arm list is deliberately explicit rather than a bare wildcard for the
            // SWARM production domain: `BranchId::SwarmMain` runs the NU6.3 rules, so Ironwood is
            // active under it, and a wildcard would silently have said otherwise.
