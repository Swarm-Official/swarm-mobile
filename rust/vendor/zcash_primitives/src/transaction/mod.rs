...
impl TxVersion {
    ...
    pub fn suggested_for_branch(consensus_branch_id: BranchId) -> Self {
        match consensus_branch_id {
            ...
            // The SWARM production domain runs the NU6.3 rules, so it takes the NU6.3 answer.
            BranchId::SwarmMain => TxVersion::V6,
        }
    }
    ...
    pub fn valid_in_branch(&self, consensus_branch_id: BranchId) -> bool {
        ...
        match self {
            ...
            TxVersion::V4 => match consensus_branch_id {
                ...
                // `SwarmMain` runs the NU6.3 rules, so it answers what NU6.3 answers. Which
                // transaction versions a SWARM *network* accepts is a network-profile rule, not
                // a property of the domain, and is enforced separately.
                Nu6_3 | SwarmMain => true,
                #[cfg(zcash_unstable = "nu7")]
                Nu7 => false, // ZIP 2003
            },
            ...
            TxVersion::V6 => match consensus_branch_id {
                ...
                | Nu6_1 | Nu6_2 => false,
                Nu6_3 | SwarmMain => true, // Ironwood / NU6.3
                #[cfg(zcash_unstable = "nu7")]
...
pub mod testing {
    ...
    pub fn arb_tx_version(branch_id: BranchId) -> impl Strategy<Value = TxVersion> {
        match branch_id {
            ...
            BranchId::SwarmMain => Just(TxVersion::V6).boxed(),
        }
    }
